...
# YOU MAY NOT INCLUDE ANY OTHER LIBRARIES, ESPECIALLY NumPy. 
# DOING SO WILL CAUSE YOU TO LOSE MARKS.

haveGlutForFonts = True   # Set to True if you have installed OpenGL GLUT so that text can be
...
from glfw import NO_WINDOW_CONTEXT
...
def buildTriangles( slice0, slice1 ):
    ...
    # Initializing the variables for the closest vertex from each slice, 
    # their respective indices, and their distance
    startVertexSlice0, startVertexSlice1  = None, None
    indexSlice0, indexSlice1, minDistance = 0, 0, 0

    # Iterating through every pair of vertices and calculating their distance (brute force method)
    for vertexSlice0 in slice0.verts:
        for vertexSlice1 in slice1.verts:

            # Calculating the distance between the pair of vertices using the vector functions provided
            currentDistance = length(subtract(vertexSlice0.coords, vertexSlice1.coords))

            # Checking to see if the current distance is less than the minimum distance
            # Also reassigns the minimum distance from 0 to the first calculated distance
            if minDistance == 0 or currentDistance < minDistance:
                minDistance = currentDistance
                startVertexSlice0, startVertexSlice1 = indexSlice0, indexSlice1
    ...
    # Creating a cyclic permutation of the vertices in slice0
    verticesSlice0 = (slice0.verts[startVertexSlice0:] 
                   + slice0.verts[:startVertexSlice0] 
                   + [slice0.verts[startVertexSlice0]])
    
    # Creating a cyclic permutation of the vertices in slice1
    verticesSlice1 = (slice1.verts[startVertexSlice1:] 
                   + slice1.verts[:startVertexSlice1] 
                   + [slice1.verts[startVertexSlice1]])
    ...
    # Initializing minArea array and minDir array to None
    minArea = [[None for col in range(len(verticesSlice0))] for row in range(len(verticesSlice1))]
    minDir  = [[None for col in range(len(verticesSlice0))] for row in range(len(verticesSlice1))]
    ...
    # Calculating the triangleArea using the vector function provided and adding the minArea from the previous column
    for col in range(1, len(verticesSlice0)):
        minArea[0][col] = triangleArea(verticesSlice1[0].coords, verticesSlice0[col].coords, verticesSlice0[col - 1].coords) + minArea[0][col - 1]
        minDir[0][col]  = Dir.PREV_COL

    # Fill in col 0 of minArea and minDir, since it's a special case as there's no col -1
    # [2 marks]

    # Calculating the triangleArea using the vector function provided and adding the minArea from the previous row
    for row in range(1, len(verticesSlice1)):
        minArea[row][0] = triangleArea(verticesSlice0[0].coords, verticesSlice1[row].coords, verticesSlice1[row - 1].coords) + minArea[row - 1][0]
    ...
    # Iterating through every (row, column) pair of vertices between slice0 and slice1
    for row in range(1, len(verticesSlice1)):
        for col in range(1, len(verticesSlice0)):
            # Calculating the triangleArea once when moving forward to the next vertex in slice0 (horizontalArea) 
            # and then again when moving forward to the next vertex in slice1 (verticalArea)
            horizontalArea = triangleArea(verticesSlice1[row].coords, verticesSlice0[col].coords, verticesSlice0[col - 1].coords) + minArea[row][col - 1]
            verticalArea   = triangleArea(verticesSlice0[col].coords, verticesSlice1[row].coords, verticesSlice1[row - 1].coords) + minArea[row - 1][col]

            # Determining the minimum of the two calculated areas to insert into the corresponding table index
            area = min(horizontalArea, verticalArea)
            minArea[row][col] = area

            # Determining the direction in which the previous minArea came from
            if area == horizontalArea:
                minDir[row][col] = Dir.PREV_COL
            else: #area == verticalArea
    ...
    # This is for your debugging, if you wish. 
    # It's not required, but is strongly recommended.
    # [0 marks]

    # print('\n') 
    # for col in range(len(verticesSlice0)):
    #     print('\t', ' ', col, end='')
    
    # print('\n')
    # for row in range(len(verticesSlice1)):
    #     print(row, '\t', end='')
    #     for col in range(len(verticesSlice0)):
    #         if minDir[row][col] == Dir.PREV_COL:
    #             direction = '-'
    #         elif minDir[row][col] == Dir.PREV_ROW:
    #             direction = '|'
    #         else:
    #             direction = '.'
    #         print('{:>4} {:>1}'.format(round(minArea[row][col]), direction), end='\t')
    #     print('\r')
    ...
    # [YOUR CODE HERE]
    # Initializing the row and column to their respective maximum indices
    row, col = len(verticesSlice1) - 1, len(verticesSlice0) - 1

    # Walking backwards through the minDir array until reaching the starting index [0][0]
    while (row != 0 or col != 0):
        triangleVertices = []

        # Determining the three triangle vertices moving backward in the direction of the previous column 
        if minDir[row][col] == Dir.PREV_COL:
            triangleVertices = [verticesSlice1[row], verticesSlice0[col], verticesSlice0[col - 1]]
            col -= 1

        # Determining the three triangle vertices moving backward in the direction of the previous row 
        else: # minDir[row][column] == Dir.PREV_ROW
            triangleVertices = [verticesSlice0[col], verticesSlice1[row], verticesSlice1[row - 1]]
            row -= 1

        # Creating a new Triangle object using the three vertices previously determined
        # and appending it to the list of triangles
        newTriangle = Triangle(triangleVertices)
        triangles.append(newTriangle)

    # Return the constructed list of triangles
    return triangles
...
# that the mouse moved. After events are processed in glfw.wait_events(), 
# check whether the mouse moved and, if so, act on it.
